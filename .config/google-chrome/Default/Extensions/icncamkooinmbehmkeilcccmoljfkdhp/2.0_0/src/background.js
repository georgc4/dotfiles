...
function optionedWA(input) {
    ...
    if (localStorage["open_in"] == "sametab") {
        chrome.tabs.getSelected( undefined, function(tab) {
            chrome.tabs.update(tab.id, {url: "https://www.wolframalpha.com/input/?i="+input}, undefined);
        });
    } else {
        chrome.tabs.create({"url" : "https://www.wolframalpha.com/input/?i=" + input, "active" : true});
    }
...
function inputChangedWA(inString, suggest) {
    ...
    var input = encodeURIComponent(inString);
    var autoUrl =  "https://www.wolframalpha.com/n/v1/api/autocomplete?i=" + input;
...
function omniWA(inString) {
    if (inString=="") {
        if (localStorage["open_in"] == "sametab") {
            chrome.tabs.getSelected( undefined, function(tab) {
                chrome.tabs.update(tab.id, {url: "https://www.wolframalpha.com/"}, undefined);
                window.close();
            });
        } else {
            chrome.tabs.create({"url" : "https://www.wolframalpha.com/", "active" : true});
        }
    } else {
        var input = encodeURIComponent(inString);
        chrome.tabs.getSelected( undefined, function(tab) {
